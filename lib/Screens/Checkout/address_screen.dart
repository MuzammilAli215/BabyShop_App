...
class _AddressScreenState extends State<AddressScreen> {
  ...
  // Index of the saved address the user has selected (-1 = none / use form)
  ...
  void initState() {
    super.initState();
    // Ensure the latest profile is loaded, then pre-fill name / phone.
    Future.microtask(() async {
      if (!mounted) return;
      final auth = context.read<AuthController>();
      // Refresh profile so we always get the latest name/phone/addresses
      await auth.fetchUserProfile();
      if (!mounted) return;
      _fullNameCtrl.text = auth.userName;
      _phoneCtrl.text = auth.userPhone;
      // If there are no saved addresses yet, go straight to the form
  ...
  // ── Apply a saved address string to the form fields ──────────────────────
  void _applySavedAddress(String raw, int index) {
    // Attempt to parse "Street, City PostalCode" or "Street, City, PostalCode"
    ...
    if (parts.length == 3) {
      ...
      if (lastParts.length > 1 &&
        ...
        _cityCtrl.text = lastParts.sublist(0, lastParts.length - 1).join(' ');
      } else {
        _cityCtrl.text = parts[1];
        _postalCtrl.text = '';
      }
  ...
  void _selectNewAddress() {
    setState(() {
      ...
      // Keep name/phone, clear address fields
      _addressCtrl.clear();
      _cityCtrl.clear();
      _postalCtrl.clear();
    });
